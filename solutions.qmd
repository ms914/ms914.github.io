...
| **5(c)** | Mit $F(x,y,z)=(x+y,\,y-x,\,2)^T$ erhält man nach Einsetzen und Kürzen: <br> $F(f(u,v))\cdot (f_u\times f_v)= e^{2u}-1$. <br> **Oberflächenintegral:** $\displaystyle \int_{u=0}^1\int_{v=0}^{2\pi}(e^{2u}-1)\,dv\,du=\pi(e^2-2)$. |
...
| **6(d)** | Sei $B$ der Boden ($z=0$) des Kugelsegments. <br> Es gilt: $\int_{\partial K}F\cdot n\,dS = \int_{\partial K\setminus B}F\cdot n\,dS+\int_B F\cdot n\,dS$. <br> Auf $B$ ist $n=(0,0,-1)$ und $F(x,y,0)=(x-y,x+y,xy)$ ⇒ $F\cdot n=-xy$. <br> Wegen Symmetrie: $\int_B (-xy)dA=0$. <br> **Ergebnis:** $\displaystyle \int_{\partial K\setminus B}F\cdot n\,dS=8\pi$. |
