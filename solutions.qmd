...
| **6(d)** | Sei $B$ der Boden ($z=0$) des Kugelsegments. <br> Es gilt: $\int_{\partial K}rot F\cdot n\,dS = \int_{\partial K\setminus B}rot F\cdot n\,dS+\int_B rot F\cdot n\,dS$. <br> Auf $B$ ist $n=(0,0,-1)$ und $rot F(x,y,0)=(x,-y,2)$ ⇒ $F\cdot n=-2$. <br> $\int_B (-2)dA=-2*2\pi$. <br> **Ergebnis:** $\displaystyle \int_{\partial K\setminus B}F\cdot n\,dS=-4\pi$. |
